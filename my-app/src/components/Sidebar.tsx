    ...
    <div className="w-64 bg-white border-r border-gray-200 flex flex-col">
      ...
      <nav className="flex-1 p-4">
        <div className="space-y-2">
          ...
          <NavItem icon={Folder} label="Boards" active>
            <div className="ml-8 mt-2 space-y-1">
              <SubNavItem label="Create routes" />
              <SubNavItem label="Development React App" />
              <SubNavItem label="Sport Xi Project" active />
              <SubNavItem label="Wordpress theme" />
            </div>
          </NavItem>
          <NavItem icon={MessageCircle} label="Messages" badge="6" />
...
function NavItem({ icon: Icon, label, active, badge, children }: NavItemProps) {
  return (
    <div>
...
function SubNavItem({ label, active }: { label: string; active?: boolean }) {
  return (
    <div className={`px-3 py-1 rounded cursor-pointer text-sm ${
